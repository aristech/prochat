...
export function getJoinEmailTemplate(sender, userEmail, team, isGuest = false) {
    ...
    return [
        ...
        `${
            isGuest
                ? "You were invited as a guest to collaborate with the team"
                : "Start collaborating with your team on ProChat"
        }`,
        "",
        `<join-link-check> Join now ( ${baseUrl}/signup_user_complete/?d=${encodeURIComponent(
            JSON.stringify({
                display_name: team.display_name.replace(" ", "+"),
                email: userEmail,
                name: team.name,
            })
        )}&t=<actual-token> )`,
...
export function getMentionEmailTemplate(
    sender,
    message,
    postId,
    siteName,
...
export function getEmailVerifyEmailTemplate(userEmail) {
    ...
    return [
        ...
        "",
        `<email-verify-link-check> Verify Email ( ${baseUrl}/do_verify_email?token=<actual-token>&email=${encodeURIComponent(
            userEmail
        )} )`,
        "",
...
export function verifyEmailBody(expectedBody, actualBody) {
    ...
    for (let i = 0; i < expectedBody.length; i++) {
        ...
        expect(
            expectedBody[i],
            `Line ${i} expects "${expectedBody[i]}" but got ${actualBody[i]}`
        ).to.equal(actualBody[i]);
    }
}
